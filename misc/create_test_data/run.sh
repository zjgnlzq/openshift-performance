...
if [ X"$choice" == X"0" ] || [ X"$choice" == X"scalable_jbossews_app" ]; then
    ...
    run_command "cd ${scalable_jbossews_app} && sed -i 's/Welcome to OpenShift/Welcome t1o OpenShift/g' src/main/webapp/index.html && git commit -a -m'modify title' && git push && cd -"
fi
